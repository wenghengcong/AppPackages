...
public extension HCFontRepresentable where Self.RawValue == Int {
    /// 返回字体
    /// - Parameter size: 字体大小
    /// - Returns: 字体
    func of(_ size: CGFloat) -> Font {
        let systemInt = HCFont.system(rawValue: self.rawValue) ?? .preferred
        guard let weightValue = HCFont.system.weight(system: systemInt) else {
 ...
 * 现有字体以及自定义字体
 * ```
 let buildInFont = HCFont.buildIn.helvetica.of(22)
 let systemFont = HCFont.system.bold.of(22)
 let customFont = HCFont.custom.XGWWenKai.bold.of(22)
 * ```
 *
 */

typealias HCSystemFont = HCFontWeightToken

public struct HCFont {
    
    public enum system: Int, HCFontRepresentable {
        /// 默认
        case preferred = 0
        /// 以下是Font.Weight
        case ultraLight
        ...
        static func weight(system: HCFont.system) -> Font.Weight? {
            switch system {
            case .preferred:
                return nil
            case .ultraLight:
    ...
    public struct custom {
        /// 一个自定义字体
        public enum XGWWenKai: String, HCFontRepresentable {
    ...
    public enum buildIn: String, HCFontRepresentable {
