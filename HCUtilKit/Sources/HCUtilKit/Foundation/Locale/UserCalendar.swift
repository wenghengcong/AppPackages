...
public class UserCalendar: ObservableObject  {
    public static let shared = UserCalendar()
    
    public let current = CalendarIdentifier.current.calendar
    
    public func weekSymbols() -> [String] {
        return current.veryShortWeekdaySymbols
    }
}

public extension Calendar {
    static var user = UserLocale.shared.calendar.current
}

public extension Date {
    
    public struct Month {
        public static func beginning(for date: Date) -> Date? {
            let startDate = date.beginning(of: .month)
            return startDate
        }
    ...
    var beginningOfMonth:Date? {
        let startDate = beginning(of: .month)
        return startDate
    }
    
    var endOfMonth:Date? {
        let startDate = end(of: .month)
        return startDate
    }
    
    var beginningOfDay:Date? {
        let startDate = beginning(of: .day)
        return startDate
    }
    
    var endOfDay:Date? {
        let startDate = end(of: .day)
        return startDate
    }
}
...
public extension Date {
    ...
    var calendar: Calendar { Calendar.user }
