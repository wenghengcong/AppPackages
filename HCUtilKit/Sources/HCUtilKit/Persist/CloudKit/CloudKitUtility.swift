...
/*
 来自：https://github.com/SwiftfulThinking/SwiftUI-Advanced-Learning
 */
...
extension CloudKitUtility {
    ...
    static var container = CoreDataStack.shared.cloudContainer
    ...
    static func discoverUser() -> Future<CKUserIdentity, Error> {
        Future { promise in
            CloudKitUtility.discoverUser { result in
                promise(result)
            }
        }
    }
    ...
    static private func discoverUser(completion: @escaping (Result<CKUserIdentity, Error>) -> ()) {
        fetchUserRecordID { fetchCompletion in
            switch fetchCompletion {
            case .success(let recordID):
                CloudKitUtility.discoverUser(id: recordID, completion: completion)
    ...
    /// - Parameters:
    ///   - id: recored id
    ///   - completion: Result<CKUserIdentity, Error>
    static private func discoverUser(id: CKRecord.ID, completion: @escaping (Result<CKUserIdentity, Error>) -> ()) {
        container.discoverUserIdentity(withUserRecordID: id) { returnedIdentity, returnedError in
            if let returnedIdentity {
                completion(.success(returnedIdentity))
            } else {
